...
def login():
    ...
    try:
        ...
        try:
            ...
            files = ["<templates folder not found>"]
        return (f"<h2>Login template failed</h2>"
                f"<p><b>Error:</b> {e}</p>"
                f"<p><b>Templates dir:</b> {tpl_dir}</p>"
                f"<p><b>Exists:</b> {os.path.isdir(tpl_dir)}</p>"
                f"<p><b>Files found:</b> {files}</p>", 500)
